...
- `ToolContext` (tools.py) carries `agent_name`/`store` so a `cacheflow_status` tool is available in the model's own tool palette (`TOOLS` in tools.py): the agent can check its own baseline/cumulative token-savings mid-loop without shelling out to a second `cf status` process and re-parsing its stdout — previously the loop had no way to know `cf`'s own CLI surface existed at all.
